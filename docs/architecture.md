...
Notification worker использует durable events/deliveries, rule-recipient links и состояние активной проблемы. Cooldown применяется после recovery, repeat interval — пока проблема активна; recovery закрывает активное состояние и отменяет pending stale deliveries. После restart записи `sending` возвращаются в `pending`. Это внутреннее состояние notification rule, а не будущая сущность Incident Mode из expansion roadmap.
