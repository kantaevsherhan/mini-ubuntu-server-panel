...
read -r -p "Remove application? [y/N] " answer
[[ "$answer" =~ ^[Yy]$ ]] || exit 0
...
read -r -p "Remove config? [y/N] " answer
[[ "$answer" =~ ^[Yy]$ ]] && rm -rf /etc/mini-ubuntu-server
read -r -p "Remove SQLite and backups? [y/N] " answer
[[ "$answer" =~ ^[Yy]$ ]] && rm -rf /var/lib/mini-ubuntu-server
